...
source_url: https://www.kiotviet.vn/huong-dan-su-dung-kiotviet/retail-xu-ly-sai-sot-hddt/gui-thong-bao-sai-sot/
---
...
Hỗ trợ
Xử lý sai sót HĐĐTGửi Thông báo sai sót
...
Tính năng Gửi Thông báo sai sót (TBSS) cho phép cửa hàng/doanh nghiệp lập và gửi thông báo lên Cơ quan Thuế (CQT) đối với các hóa đơn điện tử đã phát hành có sai sót nhưng chưa đến mức cần (hoặc chưa thể) lập hóa đơn điều chỉnh hay thay thế ngay lập tức. Hệ thống hỗ trợ xử lý cả hóa đơn được xuất từ KiotViet lẫn hóa đơn từ các phần mềm khác (Misa, Viettel...) được nhập thủ công, đồng thời cho phép gửi nhiều hóa đơn có sai sót trong cùng một lượt.

**Lợi ích nổi bật của tính năng:**

* **Xử lý sai sót linh hoạt, đúng quy định:** TBSS là cơ chế hợp lệ theo Nghị định 123/2020/NĐ-CP và 70/2025/NĐ-CP để khai báo các sai sót (thường là sai sót không ảnh hưởng đến nghĩa vụ thuế) mà không cần phải lập hóa đơn mới ngay lập tức.
* **Xử lý hàng loạt và hỗ trợ đa nền tảng:** Bạn có thể thêm nhiều hóa đơn vào cùng một thông báo để gửi đi trong một lần ký số, tiết kiệm tối đa thời gian. Việc hỗ trợ thêm cả hóa đơn ngoài hệ thống rất lý tưởng cho các doanh nghiệp đang dùng song song nhiều phần mềm kế toán.
* **Theo dõi lịch sử trực quan, minh bạch:** Tab "Thông báo sai sót" hiển thị toàn bộ lịch sử các lần gửi thông báo cho từng hóa đơn (gồm Số TBSS, Ngày, Loại, Lý do và Trạng thái phản hồi từ CQT), giúp kế toán kiểm soát tiến độ xử lý dễ dàng ngay từ danh sách.

## **II. Các thao tác cơ bản**

Để dễ hình dung, các ví dụ dưới đây sẽ minh họa những tình huống thực tế trong ngành Bán buôn, Bán lẻ.

### **1. Mở form và chọn loại thông báo**

**Tình huống:** Kế toán phát hiện một hóa đơn bán buôn cho đại lý bị ghi sai tên người nhận hàng (không ảnh hưởng mã số thuế hay tiền thuế). Bạn quyết định gửi thông báo sai sót thay vì hủy hóa đơn.

**Thao tác thực hiện như sau:**

* **Bước 1 - Điều hướng từ hóa đơn gốc**: Tại danh sách hóa đơn, nhấn vào hóa đơn bị sai để mở chi tiết → nhấn ***Xử lý hóa đơn*** (góc dưới phải) và chọn ***Gửi Thông báo sai sót***.

* **Bước 2 - Chọn Loại thông báo**: Bạn chọn 1 trong 2 loại
  + ***Người bán phát hiện sai sót***: Được chọn mặc định, không cần điền thêm thông tin.
  + ***Giải trình theo yêu cầu của CQT***.
* **Bước 3: Điền thông tin giải trình (nếu có)**: Nếu Giải trình theo yêu cầu của CQT → hệ thống sẽ hiển thị thêm 2 trường bắt buộc là Số thông báo rà soát của CQT và Ngày thông báo rà soát của CQT. Bạn cần điền đầy đủ để tránh bị báo lỗi khi gửi.

### **2. Thêm hóa đơn có sai sót vào danh sách**

**Tình huống:** Ngoài hóa đơn hiện tại, bạn phát hiện thêm một hóa đơn xuất từ tháng trước trên phần mềm khác (Misa) cũng bị sai tên công ty tương tự và muốn gửi thông báo cùng lúc.

**Thao tác thực hiện như sau:**

* **Bước 1 - Mở popup thêm hóa đơn**:
  + Trên danh sách hóa đơn → nhấn ***+ Lập hóa đơn*** → chọn ***Thông báo sai sót tới CQT***.

* Hoặc trong màn hình **Gửi thông báo sai sót** từ chi tiết hóa đơn, nhấn ***Thêm hóa đơn***.

* **Bước 2 - Thêm hóa đơn trong hệ thống (KiotViet)**: Chọn tab **KiotViet e-invoice** (mặc định chọn). Nhập số hóa đơn hoặc tên khách hàng trên ô tìm kiếm để tìm hóa đơn → chọn và nhấn ***Gửi thông báo sai sót*** để đưa vào danh sách.

* **Bước 3 - Thêm hóa đơn ngoài hệ thống**: Chọn tab **Khác (Misa, Viettel...)** → nhập thủ công các thông tin: ***Loại hóa đơn điện tử***, ***Mã của CQT***, ***Ký hiệu*** (bắt buộc), ***Số hóa đơn*** (bắt buộc) và ***Ngày hóa đơn*** → nhấn ***Gửi thông báo sai sót*** để hoàn tất.

### **3. Nhập nội dung sai sót và Ký gửi CQT**

**Tình huống:** Bạn đã gom đủ 2 hóa đơn vào danh sách và tiến hành ghi chú lỗi sai để ký gửi dữ liệu lên Cơ quan Thuế.

**Thao tác thực hiện như sau:**

* **Bước 1 - Nhập nội dung sai sót**: Tại bảng danh sách hóa đơn, điền ***Nội dung sai sót*** của từng dòng (ví dụ: "Sai tên người mua", "Sai địa chỉ"). Việc điền lý do là bắt buộc cho mọi hóa đơn có mặt trong danh sách.

* **Bước 2 - Yêu cầu Ký và gửi**: Sau khi điền đủ nội dung sai sót → nhấn ***Ký và gửi CQT***.
* **Bước 3 - Chọn Chữ ký số và hoàn tất**: Hệ thống hiển thị popup **Chọn chứng thư số** → bạn chọn ***Chữ ký số từ xa KiotViet InstrustCA*** hoặc ***Chữ ký số USB Token*** → nhấn ***Xác nhận***. Hệ thống sẽ tạo file XML và truyền lên CQT.

### **4. Kiểm tra lịch sử gửi thông báo**

**Tình huống:** Ngày hôm sau, bạn muốn kiểm tra xem Cơ quan Thuế đã chấp nhận các thông báo giải trình ngày hôm qua hay chưa.

**Thao tác thực hiện như sau:**

* **Bước 1 - Xem cảnh báo nhanh**: Tại danh sách hóa đơn, nhấn vào hóa đơn đã gửi thông báo sai sót → ở tab **Thông tin** → hệ thống hiển thị dòng thông báo cho biết hóa đơn này đã có thông báo sai sót.

* **Bước 2 - Kiểm tra chi tiết phản hồi**: Chuyển sang tab ***Thông báo sai sót*** → hệ thống liệt kê lịch sử các lần gửi thông báo sai sót của hóa đơn đó kèm theo trạng thái rõ ràng: ***Đã gửi CQT*** (đang chờ), ***CQT chấp nhận*** hoặc ***CQT không chấp nhận***.

## **III. Các tính năng nâng cao**

### **1. Lọc và theo dõi trạng thái từ Cơ quan Thuế**

**Tình huống:** Cuối tháng, kế toán trưởng yêu cầu bạn lọc ra tất cả các thông báo sai sót bị CQT từ chối để làm lại.

**Thao tác thực hiện như sau:**

* **Bước 1 - Mở bộ lọc danh sách**: Tại màn hình danh sách hóa đơn, bạn sử dụng bộ lọc bên trái và tìm đến tiêu chí ***Thông báo sai sót***.
* **Bước 2 - Lọc theo trạng thái**: Bạn có thể tích chọn nhiều trạng thái cùng lúc, đặc biệt là trạng thái ***CQT không chấp nhận***. Hệ thống sẽ lọc ra ngay lập tức các hóa đơn có thông báo sai sót bị từ chối, giúp bạn nhanh chóng vào tab **Thông báo sai sót** của từng hóa đơn để xem lý do từ chối và lập lại thông báo mới.

### **2. Chỉnh sửa/xóa hóa đơn ngoài hệ thống trong danh sách Gửi thông báo sai sót**

**Tình huống:** Bạn vừa thêm nhầm một hóa đơn từ phần mềm Misa với số hóa đơn bị nhập sai và muốn sửa lại trực tiếp trên danh sách **Gửi thông báo sai sót**.

**Thao tác thực hiện như sau:**

* Đối với hóa đơn từ phần mềm khác (Misa, Viettel...), hệ thống hỗ trợ thao tác xóa (biểu tượng thùng rác) và chỉnh sửa (biểu tượng cây bút).
* Bạn nhấn biểu tượng cây bút để sửa lại Ký hiệu, Số HĐ hoặc Ngày HĐ.

* **Lưu ý**: Hóa đơn của KiotViet chỉ có thể xóa khỏi danh sách, không thể sửa nội dung gốc*.*

### **3. Tùy chỉnh cột hiển thị**

**Tình huống:** Bạn muốn hiển thị thêm cột Mã CQT trên màn hình để tiện kiểm tra dữ liệu trước khi ký gửi.

**Thao tác thực hiện như sau**: Trên danh sách **Gửi thông báo sai sót**, bạn nhấn ***Ẩn/Hiện cột*** → tích thông tin cần hiển thị: ***Loại hóa đơn điện tử***, ***Mã CQT***.

### **4. Chuyển đổi loại thông báo**

**Tình huống:** Bạn đang làm thông báo "Người bán phát hiện" thì tìm thấy công văn rà soát của Thuế, bạn muốn đổi ngay sang dạng "Giải trình theo yêu cầu của CQT" mà không mất công làm lại từ đầu.

**Thao tác thực hiện như sau**: Trong quá trình thao tác Gửi thông báo sai sót, bạn có thể chuyển đổi linh hoạt giữa 2 loại thông báo bất kỳ lúc nào.
