...
source_url: https://www.kiotviet.vn/huong-dan-su-dung-kiotviet/retail-thiet-lap-ban-dau-hddt/lap-to-khai-dang-ky-hddt-tren-kiotviet-einvoice/
---
...
Hỗ trợ
Thiết lập ban đầu HĐĐTLập Tờ khai đăng ký HĐĐT trên KiotViet eInvoice

# **Lập tờ khai đăng ký/ thay đổi thông tin sử dụng hóa đơn điện tử**
...
#### **1.1. Phân biệt HĐĐT thông thường và HĐĐT từ máy tính tiền**
...
#### **1.2. Phân biệt Hóa đơn Bán hàng và Hóa đơn Giá trị gia tăng**
...
Tính năng này cho phép người nộp thuế lập và nộp tờ khai đăng ký hoặc thay đổi thông tin sử dụng hóa đơn điện tử (**Mẫu số 01/ĐKTĐ-HĐĐT - Phụ lục IA ban hành kèm theo Nghị định 70/2025/NĐ-CP**) trực tiếp trên phần mềm KiotViet (tích hợp trên KiotViet eInvoice). Tờ khai sẽ được ký số điện tử và gửi lên Cơ quan Thuế (CQT). Đây là bước bắt buộc trước khi cửa hàng, doanh nghiệp có thể bắt đầu phát hành hóa đơn điện tử hợp lệ, hoặc khi có bất kỳ thay đổi nào về thông tin đã đăng ký.

**Lợi ích nổi bật của tính năng:**

* **Đăng ký nhanh chóng ngay trên phần mềm:** Thay vì làm thủ tục giấy tờ phức tạp hoặc đăng nhập thêm vào cổng thuế, chủ cửa hàng có thể lập đầy đủ tờ khai Mẫu 01 và nộp lên CQT chỉ trong vài thao tác.
* **Tự động điền dữ liệu:** Hệ thống tự động lấy thông tin từ hồ sơ cửa hàng và điền lại toàn bộ thông tin người đại diện, chữ ký số, loại hóa đơn... từ tờ khai gần nhất, giúp kế toán/thu ngân hạn chế tối đa sai sót.
* **Theo dõi trạng thái phê duyệt tức thì:** Nắm bắt trạng thái duyệt của CQT (Chờ duyệt / Chấp nhận / Không chấp nhận) theo thời gian thực (real-time) để nhanh chóng bắt đầu việc xuất hóa đơn cho khách hàng.

## **II. Các thao tác cơ bản**

Để dễ hình dung, các ví dụ dưới đây sẽ minh họa những tình huống thực tế trong ngành Bán buôn, Bán lẻ.

### **1. Truy cập màn hình Tờ khai và tạo tờ khai mới**

* **Tình huống:** Cửa hàng của bạn vừa chuyển sang sử dụng Hóa đơn điện tử (HĐĐT), bạn cần mở màn hình trên KiotViet để tạo tờ khai mới gửi Cơ quan Thuế.
* **Thao tác thực hiện như sau:**
  1. Truy cập menu **Tờ khai**.
  2. Chọn **Tờ khai SD hóa đơn điện tử**

1. Hệ thống hiển thị màn hình danh sách **Tờ khai đăng ký/thay đổi thông tin sử dụng HĐĐT**.

1. Nhấn nút ***+ Tạo tờ khai***.

1. Hệ thống sẽ tự động xác định loại tờ khai:
   * Chưa có tờ khai nào trước đó: tự động chọn ***Đăng ký mới***.
   * Đã có ít nhất 1 tờ khai được CQT chấp nhận: tự động chọn ***Thay đổi thông tin***.
   * Bạn có thể chuyển đổi loại tờ khai theo đúng thực tế cần gửi CQT.

### **2. Kiểm tra và xác nhận thông tin đơn vị**

* **Tình huống:** Bạn cần đảm bảo thông tin pháp lý của cửa hàng được điền chính xác trên tờ khai gửi CQT.
* **Thao tác thực hiện như sau:**
  1. Kiểm tra phần **Thông tin đơn vị** đã được tự động điền, bao gồm:
     + Tên đơn vị
     + Mã số thuế
...
1. Nếu thấy thông tin chưa chính xác, bạn nhấn vào ***Thông tin đơn vị***.

1. Hệ thống chuyển sang màn hình **Thiết lập** → **Thông tin đơn vị,** bạn nhấn ***Chỉnh sửa*** để cập nhật hồ sơ, sau đó quay lại để tiếp tục tạo tờ khai.

**Lưu ý**: Đối với Hộ/cá nhân kinh doanh, Mã số thuế sẽ được lấy từ CCCD của người đại diện. Khi nộp tờ khai, hệ thống sẽ kiểm tra đảm bảo CCCD phải khớp với MST.

### **3. Khai thông tin người đại diện pháp luật**

* **Tình huống:** Khai báo thông tin của bạn (chủ cửa hàng/người đại diện pháp luật) - người sẽ chịu trách nhiệm chính đối với tờ khai này.
* **Thao tác thực hiện như sau:** Tại khu vực **Thông tin người đại diện pháp luật**, điền đầy đủ các trường bắt buộc. Nếu đã có tờ khai trước: các trường thông tin này được tự động điền từ tờ khai gần nhất, bạn kiểm tra và cập nhật nếu có thay đổi. Các thông tin bao gồm:
  + Họ và tên (tối đa 50 ký tự)
  + Số CCCD (12 chữ số)
  + Ngày sinh (hợp lệ, năm từ 1900 đến năm hiện tại)
  + Giới tính
  + Số điện thoại
  + Email (đúng định dạng, tối đa 50 ký tự)
  + Số hộ chiếu không bắt buộc.

### **4. Cấu hình thông tin hóa đơn**

* **Tình huống:** Bạn cần thiết lập loại hóa đơn và cách thức truyền dữ liệu phù hợp với quy mô và mô hình kinh doanh bán lẻ của mình.
* **Thao tác thực hiện như sau:**
  1. **Hình thức hóa đơn**: Tích chọn ít nhất 1 hình thức:
     + ***Có mã của cơ quan thuế***
     + ***Không có mã của cơ quan thuế***
     + ***Hóa đơn có mã được khởi tạo từ máy tính tiền***
  2. **Hình thức gửi dữ liệu hóa đơn điện tử**: Nội dung tự động thay đổi theo hình thức hóa đơn đã chọn.
     + Khi chọn ***Có mã của cơ quan thuế*** hoặc ***Hóa đơn có mã được khởi tạo từ máy tính tiền***: Hiển thị nội dung HĐĐT có mã với 3 lựa chọn bổ sung không bắt buộc (dành cho doanh nghiệp nhỏ và vừa khu vực khó khăn, doanh nghiệp theo đề nghị UBND, cơ quan quản lý tài sản công).
     + Khi chỉ chọn ***Không có mã của cơ quan thuế***: Mặc định chọn gửi qua tổ chức cung cấp dịch vụ, không được thay đổi.
  3. **Phương thức chuyển dữ liệu hóa đơn cho CQT**: Lựa chọn 1 trong 2 phương thức:
     + Chuyển đầy đủ nội dung từng hóa đơn (mặc định cho hầu hết)
     + Chuyển dữ liệu hóa đơn điện tử theo Bảng tổng hợp dữ liệu hóa đơn điện tử theo Mẫu số 01/TH-HĐĐT.

**Lưu ý**: Với hình thức hóa đơn ***Có mã của cơ quan thuế*** → bắt buộc chọn phương thức ***Chuyển đầy đủ nội dung từng hóa đơn***.

1. **Loại hóa đơn sử dụng**: Tích chọn loại hóa đơn sử dụng phù hợp.

**Lưu ý**: KiotViet sẽ tự động chặn chọn các loại hóa đơn không phù hợp với hình thức kê khai thuế (Trực tiếp/Khấu trừ) của cửa hàng.

### **5. Khai thông tin chữ ký số**

* **Tình huống:** Khai báo chữ ký số (CKS) mà cửa hàng sẽ sử dụng để ký hóa đơn điện tử.
* **Thao tác thực hiện như sau:**
  1. Hệ thống tự động hiển thị danh sách CKS liên kết với tài khoản.
  2. Nhấn ***+ Thêm chữ ký số*** để bổ sung nếu cần.
  3. Chọn ***Hình thức đăng ký*** tương ứng cho từng dòng CKS:
     + ***Thêm mới***: Khi CKS chưa từng được khai báo với cơ quan Thuế trước đó.
     + ***Gia hạn***: Khi CKS gần hết hạn, gia hạn thêm.
     + ***Ngừng sử dụng***: Khi không dùng CKS đã khai báo với cơ quan Thuế trước đó

**Lưu ý**: Danh sách CKS không được để trống và ***Số serial CKS*** phải khớp với MST của cửa hàng. Riêng đối với CKS RMS, hệ thống sẽ kiểm tra MST đăng ký chữ ký số cần khớp với MST đăng ký trên tờ khai.

### **6. Khai thông tin khác**

* **Tình huống:** Bổ sung thông tin về các đơn vị cung cấp giải pháp, trong trường hợp cửa hàng có sử dụng thêm phần mềm truyền nhận dữ liệu khác ngoài KiotViet.
* **Thao tác thực hiện như sau:**
  1. Kiểm tra bảng **Thông tin tổ chức cung cấp phần mềm hóa đơn điện tử** và **Thông tin tổ chức truyền nhận dữ liệu hóa đơn điện tử**.
  2. Nhấn ***+ Thêm dòng*** nếu bạn sử dụng thêm tổ chức khác ngoài KiotViet.
  3. Chọn ***Có*** hoặc ***Không*** cho phần ***đơn vị hạch toán phụ thuộc cần cấp quyền tra cứu hóa đơn trên trang của thuế*** hoặc ***Tạm ngưng sử dụng hóa đơn điện tử***.

**Lưu ý**: Dòng mặc định KiotViet trong các bảng tổ chức là cố định và không thể xóa. Nếu tờ khai gần nhất đã có thêm tổ chức khác, hệ thống tự động điền lại.

### **7. Xác nhận, ký số và nộp tờ khai cho CQT**

* **Tình huống:** Bạn đã hoàn tất việc kiểm tra và tiến hành ký điện tử để chính thức gửi tờ khai của cửa hàng lên Cơ quan Thuế.
* **Thao tác thực hiện như sau:**
  1. Nhấn nút ***Nộp tờ khai cho CQT***.
  2. Bạn rà soát lại Email và Tên đơn vị một lần nữa, sau đó nhấn ***Tiếp tục***.

1. Chọn loại CKS cửa hàng đang dùng (IntrustCA hoặc USB Token) và nhấn ***Xác nhận***.

1. Nhấn ***Đồng ý*** tại màn hình thông báo thành công để quay về danh sách quản lý tờ khai.

### **8. Theo dõi trạng thái tờ khai trong danh sách**

* **Tình huống:** Kiểm tra xem Cơ quan Thuế đã duyệt tờ khai chưa để cửa hàng có thể bắt đầu xuất hóa đơn điện tử cho khách mua hàng.
* **Thao tác thực hiện như sau:**
  1. Tại màn hình danh sách tờ khai, theo dõi cột **Trạng thái** (Chờ CQT duyệt/CQT Chấp nhận/CQT không chấp nhận).

1. Khi trạng thái chuyển thành ***CQT Chấp nhận***, bạn kiểm tra ***Mã CQT cấp*** nếu đăng ký sử dụng hóa đơn khởi tạo từ máy tính tiền để dùng cho việc cấu hình phát hành hóa đơn.

1. Nếu trạng thái là ***CQT không chấp nhận***, nhấn ***Xem chi tiết*** để biết lý do từ chối và tiến hành tạo tờ khai điều chỉnh.

## **III. Các tính năng nâng cao**

### **1. Tạo tờ khai thay đổi thông tin**

* **Tình huống:** Cửa hàng của bạn vừa gia hạn chữ ký số hoặc thay đổi nhà cung cấp CKS, bạn cần làm tờ khai cập nhật thông tin mới với CQT.
* **Thao tác thực hiện như sau:**
  1. Nhấn nút ***+ Tạo tờ khai***. Hệ thống sẽ tự động chuyển sang chế độ ***Thay đổi thông tin*** (nếu đã có tờ khai trước). Toàn bộ thông tin từ tờ khai gần nhất được điền sẵn.
  2. Chỉnh sửa các trường thông tin cần thay đổi.
  3. Thực hiện ký số và nộp lại cho CQT tương tự như quy trình tạo mới.

### **2. Tự động điền dữ liệu từ tờ khai gần nhất**

* **Tình huống:** Giúp bạn rút ngắn tối đa thời gian nhập liệu khi cần tạo tờ khai mới hoặc phải làm lại tờ khai do bị CQT từ chối trước đó.
* **Thao tác thực hiện như sau:** Tính năng này hoạt động tự động.
  + Hệ thống sẽ điền sẵn các trường: Họ và tên, Số CCCD, Ngày sinh, Số điện thoại, Số hộ chiếu, Giới tính, Email, Hình thức hóa đơn, Hình thức gửi dữ liệu hóa đơn điện tử, Phương thức chuyển dữ liệu cho CQT, Thông tin chữ ký số, Thông tin tổ chức cung cấp phần mềm HĐĐT, Thông tin tổ chức truyền nhận dữ liệu HĐĐT, Có đơn vị hạch toán phụ thuộc.
  + **Lưu ý**: Thông tin đơn vị luôn lấy từ **Thiết lập** → **Thông tin đơn vị** hiện tại, không phụ thuộc tờ khai cũ.

### **3. Kiểm soát logic hình thức hóa đơn theo loại hình đơn vị**

* **Tình huống:** Đảm bảo tờ khai của cửa hàng luôn hợp lệ và tuân thủ đúng quy định pháp luật dựa trên mô hình kinh doanh hiện tại.
* **Thao tác thực hiện như sau:** Hệ thống tự động vô hiệu hóa hoặc chọn cứng các tùy chọn phù hợp. Cụ thể:
  + Cửa hàng kê khai thuế Trực tiếp: Không được chọn Hóa đơn giá trị gia tăng.
  + Cửa hàng kê khai thuế Khấu trừ: Không được chọn Hóa đơn bán hàng.
  + Loại hình Hộ/cá nhân kinh doanh: Mặc định chọn cứng Có mã của cơ quan thuế.

### **4. Quản lý danh sách chữ ký số**

* **Tình huống:** Thêm, xóa hoặc cập nhật trạng thái của các chữ ký số trong suốt quá trình hoạt động của cửa hàng.
* **Thao tác thực hiện như sau:**
  1. **Thêm mới CKS**: Nhấn ***+ Thêm chữ ký số*** để tạo thêm dòng nhập liệu mới → chọn 1 trong 2 tùy chọn:

* ***Thêm các chữ ký số đang chờ đăng ký***: Hệ thống tự động điền thông tin của CKS có ***Trạng thái CQT*** → ***Chưa đăng ký*** trong danh sách **Thiết lập** → **Chữ ký số**.

* ***Thêm chữ ký số mới***: Chọn 1 trong 3 loại CKS → nhấn ***Tiếp tục***.

Tùy từng loại CKS → hệ thống hiển thị các thông tin tương ứng, cụ thể như sau:

* ***Chữ ký số từ xa KiotViet IntrustCA***:
  + Bạn điền thông tin ***Tên đăng nhập (UUID)*** được IntrustCA cấp → nhấn enter → hệ thống tự động điền các thông tin ***Chứng thư số***, ***Hiệu lực từ ngày***, ***Hiệu lực đến ngày***. Bạn chọn hình thức đăng ký → nhấn ***Tiếp tục****.*

* Chọn 1 trong 5 phương thức xác thực trước khi ký hóa đơn.

* ***Nhập thủ công mật khẩu và mã PIN***.
...
* ***Xác thực qua ứng dụng IntrustCA***: Kiểm tra ứng dụng IntrustCA → mở ứng dụng để xác nhận.
...
* **Lưu ý**: Chữ ký số được thêm tại đây được đồng bộ vào danh sách chữ ký số tại màn hình **Thiết lập** → **Chữ ký số**.

* ***Chữ ký USB token***: Bạn thao tác cài đặt theo 3 bước hướng dẫn trên màn hình → nhấn ***Xác nhận***:
  + Bước 1: Cắm USB Token vào máy tính
  + Bước 2: Cài đặt Token Manager cho máy tính
  + Bước 3: Nhấn ***Cài đặt tại đây*** để Cài đặt thiết bị ký số KV-Sign cho máy tính của bạn.

* ***Chữ ký số nhà cung cấp khác***: Bạn điền các thông tin → nhấn ***Xác nhận***, bao gồm:
  + ***Tổ chức chứng thực chữ ký số***
  + ***Số serial chữ ký số***
  + ***Hiệu lực từ ngày/ đến ngày***
  + ***Hình thức đăng ký***

1. Cập nhật trạng thái CKS: Trên dòng thông tin CKS, bạn chọn hình thức đăng ký phù hợp với nhu cầu: ***Thêm mới***/***Gia hạn***/***Ngừng sử dụng***.

1. Nhấn biểu tượng thùng rác → chọn ***Xác nhận*** để xóa các dòng chữ ký số bạn tự thêm nếu thao tác sai.

### **5. Ghi nhận lịch sử thao tác**

* **Tình huống:** Chủ cửa hàng cần kiểm tra lại xem nhân viên hoặc kế toán nào đã thao tác tạo/gửi tờ khai và vào thời gian nào để kiểm soát luồng công việc.
* **Thao tác thực hiện như sau:** Mỗi khi tờ khai được tạo thành công, hệ thống sẽ tự động lưu lại **Lịch sử thao tác** bao gồm: Tên người dùng thao tác, Hành động (Đăng ký mới/Thay đổi thông tin), Thời gian thực hiện và IP thiết bị.
