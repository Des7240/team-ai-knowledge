...
source_url: https://www.kiotviet.vn/huong-dan-su-dung-kiotviet/retail-thiet-lap-ban-dau-hddt/xu-ly-loi-to-khai-dang-ky-hddt-thuong-gap/
---
...
Hỗ trợ
Thiết lập ban đầu HĐĐTXử lý lỗi tờ khai đăng ký HĐĐT thường gặp

# **Xử lý lỗi tờ khai đăng ký HĐĐT thường gặp**

*(Ngành Bán buôn, Bán lẻ*

Trong quá trình nộp tờ khai đăng ký sử dụng hóa đơn điện tử, người dùng thường gặp một số lỗi phổ biến.

Sau đây là nguyên nhân và cách khắc phục

* ## Lỗi 1008: Trạng thái MST của NNT không hợp lệ

  Khi gặp lỗi này, đơn vị cần tra cứu lại tình trạng hoạt động của Người nộp thuế (NNT) tại trang tra cứu NNT https://tracuunnt.gdt.gov.vn/

  *- Trường hợp 1:* Mã số thuế có trạng thái **khác** NNT đang hoạt động

  **Cách xử lý**: đơn vị cần liên hệ với Cơ quan Thuế để cập nhật lại trạng thái MST trước, sau đó mới lập lại tờ khai gửi CQT.

  *- Trường hợp 2:*Hộ kinh doanh có nhiều MST cũ (MST-001, MST-888...) trong đó có một số MST đã ngừng hoạt động, hệ thống của Tổng cục thuế đang ghi nhận trạng thái của đơn vị theo trạng thái của mã số thuế ngừng hoạt động

  + Chưa đồng bộ thông tin về CCCD
  + **2 cách xử lý**:
    - **Cách 1**: Chờ hệ thống đồng bộ thông tin của đơn vị về trạng thái đúng
    - **Cách 2**: Đăng ký lại theo mã số thuế cũ đang hoạt động
* ## Lỗi 1009: Cơ quan thuế quản lý của NNT không hợp lệ

  **Cách xử lý**: Khi gặp lỗi này, đơn vị cần tra cứu thông tin **Cơ quan thuế quản lý** trên trang https://tracuunnt.gdt.gov.vn/ cũng như kiểm tra lại thông tin trên phần mềm **KiotViet E-Invoice**. Nếu phát hiện thông tin giữa hai nguồn này không khớp nhau, bạn chủ động liên hệ trực tiếp với Cơ quan Thuế để được hỗ trợ xử lý.

  **Các nguyên nhân thường gặp**.

  *- Trường hợp 1:*Chọn nhầm cơ quan thuế trùng tên

  **Nguyên nhân**: Nhiều cơ quan thuế có cùng tên nhưng mã khác nhau.

  **Ví dụ**: Cùng tên "Thuế cơ sở 1 tỉnh Cà Mau" nhưng có:

  + Mã 82311: Quản lý các xã Tân Thuận, Tân Tiến...
  + Mã 82301: Quản lý các phường An Xuyên, Lý Văn Lâm...

  **Cách khắc phục**: Chọn đúng cơ quan thuế có gắn với phường/xã thuộc địa chỉ khách hàng

  *- Trường hợp 2:*Mã cơ quan thuế trong CSDL khác với thực tế

  **Nguyên nhân**:

  + Hệ thống thuế đang chuyển đổi từ mã số thuế cũ sang số CCCD
  + CSDL có thể lưu mã cơ quan thuế của cá nhân chủ hộ thay vì hộ kinh doanh
  + Trên https://tracuunnt.gdt.gov.vn/ vẫn hiển thị dạng "MSTcũ-001"

  **Cách khắc phục**:

  + **Cách 1: Chờ hệ thống cập nhật** **mã số thuế thành số CCCD**
  + **Cách 2: Sử dụng lại mã số thuế cũ (đang hoạt động)****không dùng CCCD,​****để:**
    - Đăng ký lại tài khoản hóa đơn điện tử theo mã số thuế 001 (loại hình: Doanh nghiệp)
    - Đăng ký lại chữ ký số cho mã số thuế 001
    - Đăng ký lại tờ khai theo đúng cơ quan thuế trên trang https://tracuunnt.gdt.gov.vn/

  *Lưu ý: Trường hợp này bạn cần liên hệ KiotViet để được đổi thông tin mã số thuế từ CCCD về MST cũ - 001*​​
* ## Lỗi 1010: NNT chưa đăng ký sử dụng hóa đơn điện tử

  **Nguyên nhân:** Tờ khai đang gửi với hình thức “Đăng ký thay đổi” mà trước đó đơn vị chưa có tờ khai Đăng ký mới nào được chấp nhận

  **Cách xử lý đơn giản**: Lập tờ khai với hình thức "Đăng ký mới"
* ## Lỗi 1011: Thông tin chữ ký số không đúng định dạng

  **Nguyên nhân**: MST trong chữ ký số và MST tờ khai không khớp

  **Cách khắc phục**: Đảm bảo MST trong tờ khai và chữ ký số phải giống nhau

  **Ví dụ**: Tờ khai dùng CCCD 12 số → chữ ký số cũng phải đăng ký theo CCCD 12 số
* ## Lỗi 1012: NNT không thuộc đối tượng sử dụng hóa đơn điện tử

  **Nguyên nhân**: Do thông tin Chứng minh thư (CMT) của đơn vị trên Tờ khai là CCCD trên trang https://tracuunnt.gdt.gov.vn/ thì là Mã số thuế cũ và CMND.

  **Cách xử lý**:

  + **Cách 1**: Liên hệ quản lý Thuế để cập nhật thông tin Chứng minh thư (CMT) thành CCCD mới.
  + **Cách 2**: Liên hệ KiotViet ghi rõ chuyển lại dùng KiotViet E-Invoice theo Mã số thuế cũ. Lưu ý: nếu Chữ ký số của bạn cũng là CCCD thì cũng cần báo NCC chữ ký số chuyển lại về Mã số thuế cũ.
* ## Lỗi 1025: Loại hóa đơn sử dụng không hợp lệ

  **Mô tả lỗi**: NNT chọn loại hóa đơn sử dụng chưa phù hợp với phương pháp tính thuế đã đăng ký với cơ quan thuế.

  **Nguyên nhân**: Do chọn sai loại Hóa đơn sử dụng trên Tờ khai.

  **Cách xử lý**: Truy cập vào trang https://tracuunnt.gdt.gov.vn/ để tra cứu theo Mã số thuế, sau đó tích vào Tên Công ty hoặc Hộ kinh doanh, kiểm tra ô **PP tính thuế GTGT**:

  + Nếu PP Tính thuế là **khấu trừ** thì trên tờ khai phải tích Loại hóa đơn sử dụng là **Hoá đơn GTGT**.
  + Nếu PP tính thuế là **khoán**, **trực tiếp** thì trên tờ khai phải tích Loại hóa đơn sử dụng là **Hoá đơn bán hàng**.
* ## Lỗi 1027: Tổ chức truyền nhận không hợp lệ

  **Tình trạng**: Cơ quan thuế không tiếp nhận tờ khai với lý do là: 1027 – Tổ chức truyền nhận không hợp lệ. Thông tin **Từ ngày** phải lớn hơn hoặc bằng ngày bắt đầu cung cấp dịch vụ của tổ chức truyền nhận…

  **Nguyên nhân**: Do ngày điền trên tờ khai nhỏ hơn ngày KiotViet bắt đầu cung cấp dịch vụ truyền nhận hóa đơn điện tử trên hệ thống Cơ quan thuế.

  **Cách xử lý**:  Kiểm tra lại thông tin **Từ ngày** đã khai báo tại mục **Thông tin đơn vị truyền nhận** trên tờ khai.
* ## Lỗi 1031: Hộ, cá nhân kinh doanh chưa có giấy chứng nhận đăng ký kinh doanh

  - **Nguyên nhân**: Trang tra cứu https://tracuunnt.gdt.gov.vn/ không có thông tin đăng ký Kinh doanh.

  **- Cách xử lý**:

  + Truy cập vào trang https://tracuunnt.gdt.gov.vn/ để tra cứu theo Mã số thuế, sau đó tích vào Tên hộ kinh doanh, xem ô **GPKD-Ngày cấp** nếu còn trống thì liên hệ Cơ quan thuế để kiểm tra.
  + Sau khi được Cơ quan Thuế đã cập nhật thông tin thì bạn lập lại Tờ khai khác và gửi Cơ quan Thuế.
* ## Lỗi 1034: Thông tin Người đại diện pháp luật không đúng

  **Cách xử lý**: Khi gặp lỗi này, đơn vị cần đối chiếu thông tin Người đại diện pháp luật trên tờ khai với thông tin trong hệ thống của cơ quan chức năng để đảm bảo tính chính xác.

  **Các trường hợp thường gặp:**

  *- Trường hợp 1:*Thông tin người đại diện pháp luật không đúng so với app VNeID

  **Cách xử lý chung**: Nếu gặp lỗi trên, Người nộp thuế cần thực hiện: Đối chiếu lại thông tin Người đại diện theo pháp luật tại app VNeID với thông tin Người đại diện theo pháp luật trên tờ khai đã khớp hay chưa.

  + **Bước 1: Kiểm tra thông tin Người đại diện theo pháp luật tại app VNeID.**
    - Đăng nhập app VNeID.
    - Nhấn **Định danh tổ chức**.
  + Chọn công ty đang hoạt động đã được định danh doanh nghiệp.
  + Chọn mục **Thông tin tổ chức**.
  + Kiểm tra thông tin Người đại diện theo pháp luật tại mục **THÔNG TIN NGƯỜI ĐẠI DIỆN PHÁP LUẬT HOẶC NGƯỜI ĐỨNG ĐẦU TỔ CHỨC**.
  + **Bước 2: Kiểm tra Thông tin Người đại diện theo pháp luật trên tờ khai**: **​**Thông tin Người đại diện theo pháp luật trên tờ khai phải khớp với thông tin tại app VNeID, trường hợp chưa khớp, đơn vị cần cập nhật lại thông tin nêu trên tại Hồ sơ hóa đơn điện tử trên phần mềm KiotViet E-Invoice, sau đó lập lại tờ khai gửi Cơ quan thuế.

  *- Trường hợp 2:*Thông tin liên quan đến người đại diện pháp luật không đúng so với Đăng ký doanh nghiệp/Đăng ký thuế.

  **Cách xử lý**: Nếu gặp lỗi này, đơn vị cần thực hiện: Đối chiếu lại thông tin Người đại diện theo pháp luật trên tờ khai đã khớp với thông tin tại Đăng ký doanh nghiệp và đăng ký thuế hay chưa

  + **Bước 1: Kiểm tra Thông tin Người đại diện theo pháp luật trên tờ khai.**
  + **Bước 2: Kiểm tra lại thông tin Người đại diện theo pháp luật tại tờ khai đăng ký sử dụng hóa đơn điện tử đã khớp với đăng ký kinh doanh, đăng ký thuế hay chưa.**
    - Kiểm tra thông tin Người đại diện theo pháp luật tại tờ khai trên giấy đăng ký doanh nghiệp (kiểm tra trên bản cứng)
    - Đối với Hộ kinh doanh, kiểm tra thông tin tại đăng ký kinh doanh, đăng ký thuế (kiểm tra trên bản cứng)
    - Thông tin Người đại diện theo pháp luật trên tờ khai phải khớp với thông tin tại đăng ký kinh doanh, khớp với thông tin tại đăng ký thuế. Trường hợp chưa khớp, đơn vị cần cập nhật lại thông tin nêu trên tại mục **Thông tin đơn vị**, sau đó lập lại tờ khai gửi Cơ quan thuế.

  **Các nguyên nhân gây sai lệch thông tin thường gặp:**

  + **Nguyên nhân 1**: Hộ kinh doanh chưa được cập nhật MST thành số CCCD
    - **Cách xử lý**:
      * **Cách 1**: Chờ hệ thống cập nhật MST thành CCCD
      * **Cách 2**: Đăng ký lại theo mã số thuế cũ 001
  + **Nguyên nhân 2:** Thông tin đăng ký vẫn là CMND cũ
    - **Cách xử lý**: Liên hệ cán bộ thuế/phòng đăng ký kinh doanh để cập nhật thành số CCCD

  *- Trường hợp 3:*1034 – Thông tin liên quan đến người đại diện pháp luật không khớp đúng so với Đăng ký doanh nghiệp/Đăng ký thuế.. Tên đại diện pháp luật

  **Lưu ý**: Tên phải giống với Đại diện Pháp luật trang tra cứu Người nộp thuế.

  + Chữ viết Hoa hay viết thường phải giống trên trang đó.
  + Nếu có dấu phẩy hay cách thì cũng phải khai báo đúng như vậy.
  + Với đơn vị là **Hộ kinh doanh**: nếu trên trang tra cứu Người nộp thuế là Mã số thuế cũ mà Tờ khai lập trên KiotViet E-Invoice là số Căn cước công dân thì phải cập nhật trên KiotViet E-Invoice cũng phải về là Mã số thuế cũ, sau đó mới lập Tờ khai.

  *- Trường hợp 4:*1034 – Thông tin liên quan đến người đại diện pháp luật không khớp đúng so với Đăng ký doanh nghiệp/Đăng ký thuế.. Ngày sinh

  + Nếu kiểm tra thấy đã khớp đúng Ngày/Tháng/Năm sinh rồi thì Liên hệ KiotViet  cung cấp thông tin Đăng ký kinh doanh của đơn vị để KiotViet kiểm tra với Thuế.
  + **Lưu ý:** Thông tin này chỉ hỗ trợ với khách hàng Doanh nghiệp, vì vậy với Hộ kinh doanh thì liên hệ quản lý Thuế để kiểm tra Ngày/Tháng/Năm sinh
* ## Lỗi 2007: Hình thức đăng ký chưa đúng

  **Mô tả lỗi**: Đề nghị NNT bổ sung thông tin người liên hệ, điện thoại liên hệ, ngõ, ngách, mặt đường thì ghi tên đường.

  **Cách xử lý**: Đây là thông báo từ chối từ Cơ quan quản lý Thuế, vì vậy người nộp thuế cần liên hệ quản lý Thuế.
* ## Lỗi 2013: Thông tin mã số thuế không khớp trong chữ ký số

  **Nguyên nhân**: Do thông tin Chứng minh thư (CMT) của đơn vị trên Tờ khai là CCCD trên trang https://tracuunnt.gdt.gov.vn/ là Mã số thuế cũ và CMND.

  **Cách xử lý**:

  + **Cách 1**: Liên hệ quản lý Thuế để cập nhật thông tin Chứng minh thư (CMT) thành CCCD mới.
  + **Cách 2**: Liên hệ KiotViet ghi rõ chuyển lại dùng KiotViet E-Invoice theo Mã số thuế cũ. Lưu ý: nếu Chữ ký số của bạn cũng là CCCD thì cũng cần báo NCC chữ ký số chuyển lại về Mã số thuế cũ.
* ## Lỗi 9991: Cơ quan thuế đang xử lý tờ khai của người nộp thuế

  **Nguyên nhân**: Do trên trang https://hoadondientu.gdt.gov.vn có Tờ khai đang được xử lý.

  **Cách xử lý**:

  + **Bước 1**: Vào trang https://hoadondientu.gdt.gov.vn và đăng nhập/ nhấn mục **Quản lý danh mục**/ **Thông báo từ CQT**/, chọn tab **Thông báo 01/TB-ĐKĐT**/, kiểm tra hiện tại đang không có Tờ khai nào được chấp nhận
  + **Bước 2**: Liên hệ quản lý thuế để nhờ duyệt Tờ khai.

  **Lưu ý**: Nếu Đơn vị mới thành lập và chưa có tài khoản trên trang https://hoadondientu.gdt.gov.vn thì liên hệ quản lý Thuế duyệt Tờ khai.
* ## Lỗi 9992: Đã tồn tại tờ khai đăng ký sử dụng hóa đơn điện tử khác của người nộp thuế đã được cơ quan thuế chấp nhận

  *- Nguyên nhân 1:*Do trên hệ thống Thuế đang có Tờ khai chờ xử lý chưa được duyệt

  **Cách xử lý**: 

  + **Bước 1**: Vào trang https://hoadondientu.gdt.gov.vn và đăng nhập/ nhấn mục **Quản lý danh mục**/ **Thông báo từ CQT**/ chọn sang tab **Thông báo 01/TB-ĐKĐT**/ kiểm tra sẽ không thấy tờ khai được chấp nhận
  + **Bước 2**: Liên hệ quản lý thuế để nhờ duyệt Tờ khai.

  *- Nguyên nhân 2:*Do bạn đã có Tờ khai được CQT chấp nhận rồi nhưng lại lập Tờ khai chọn là Đăng ký mới

  **Cách xử lý**: Lập Tờ khai chọn là loại **Thay đổi thông tin**.
* ## Lỗi 9994: Ngày đăng ký sử dụng HĐĐT thuộc thời gian ngừng sử dụng HĐĐT

  Khi gặp lỗi này, đơn vị cần tra cứu lại tình trạng hoạt động của Người nộp thuế tại trang https://tracuunnt.gdt.gov.vn/

  *- Trường hợp 1:* Bị cưỡng chế hóa đơn

  **Kiểm tra**: Tra cứu tại https://www.gdt.gov.vn/wps/portal/home/qdcchd1

  **Cách xử lý**: Chờ hết thời hạn cưỡng chế -> gửi lại tờ khai

  *- Trường hợp 2:*MST không ở trạng thái "đang hoạt động"

  Kiểm tra trạng thái của mã số thuế tại trang: https://tracuunnt.gdt.gov.vn/tcnnt/mstdn.jsp

  **Cách xử lý**: Liên hệ cán bộ thuế cập nhật trạng thái -> gửi lại tờ khai
