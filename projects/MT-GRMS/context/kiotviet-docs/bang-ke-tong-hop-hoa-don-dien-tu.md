...
source_url: https://www.kiotviet.vn/huong-dan-su-dung-kiotviet/retail-phat-hanh-hddt/bang-ke-tong-hop-hoa-don-dien-tu/
---
...
Hỗ trợ
Phát hành HĐĐTBảng kê tổng hợp hóa đơn điện tử
...
Tính năng Báo cáo cung cấp 4 loại bảng kê hóa đơn điện tử phục vụ kê khai thuế, đối soát và kiểm tra nội bộ. Bảng kê tổng hợp là báo cáo cốt lõi, liệt kê toàn bộ hóa đơn đã phát hành trong kỳ với đầy đủ thông tin định danh, khách hàng, trạng thái CQT và giá trị. Ngoài ra còn có Bảng kê hóa đơn điều chỉnh, Bảng kê hóa đơn thay thế và Bảng kê hóa đơn lập thông báo sai sót. Mỗi báo cáo đều hỗ trợ lọc đa điều kiện, tìm kiếm nhanh và xuất file Excel (.xlsx) chuẩn xác.

**Lợi ích nổi bật của tính năng:**

* **Toàn bộ dữ liệu hóa đơn trong một màn hình:** Bạn có cái nhìn toàn diện để đối chiếu và kiểm tra. Bảng kê tổng hợp hiển thị đầy đủ tất cả hóa đơn đã phát hành, bao gồm cả hóa đơn gốc, điều chỉnh, thay thế, phát hành lỗi và CQT từ chối, không loại trừ bất kỳ trạng thái nào.
* **Lọc đa điều kiện linh hoạt với logic chính xác:** Bộ lọc "Kỳ xuất hóa đơn" sử dụng ngày xuất (ngày phát hành thực tế) thay vì ngày tạo nháp, đảm bảo dữ liệu kỳ khai thuế chuẩn xác. Bộ lọc trạng thái tự động thích ứng với từng hình thức hóa đơn cửa hàng đang dùng.
* **Xuất file Excel chuẩn sẵn sàng cho kê khai thuế:** Hỗ trợ xuất file "Tổng quan" và "Chi tiết". File xuất tuân thủ đúng điều kiện đang lọc và được đặt tên tự động, giúp việc báo cáo thuế của cửa hàng trở nên nhẹ nhàng, không bị thừa dữ liệu.

## **II. Các thao tác cơ bản**

Để dễ hình dung, các ví dụ dưới đây sẽ minh họa những tình huống thực tế trong ngành Bán buôn, Bán lẻ.

### **1. Truy cập, xem và tìm kiếm hóa đơn trên Bảng kê tổng hợp**

**Tình huống:** Cuối tháng, bạn cần kiểm tra lại toàn bộ hóa đơn điện tử đã xuất cho khách sỉ và khách lẻ tại cửa hàng để đối chiếu doanh thu thực tế.

**Thao tác thực hiện như sau:**

* **Truy cập Bảng kê tổng hợp**:
  + Truy cập menu **Báo cáo**.
  + Chọn **Bảng kê tổng hợp** để chuyển đến màn hình báo cáo.
  + **Lưu ý**: Để truy cập vào báo cáo bạn cần có quyền Báo cáo - Bảng kê tổng hợp.

* **Xem danh sách hóa đơn**:
  + Màn hình hiển thị toàn bộ hóa đơn đã phát hành trong kỳ (gồm gốc, điều chỉnh, thay thế, phát hành lỗi, CQT từ chối).
  + Cột ***Ngày hóa đơn*** là ngày xuất thực tế.
  + **Lưu ý**: Báo cáo chỉ hiển thị hóa đơn thuộc các hình thức đã đăng ký và được CQT chấp nhận; các hình thức bị từ chối tờ khai sẽ không xuất hiện.

* **Lọc dữ liệu theo nhu cầu**: Tại bộ lọc bên trái, bạn có thể kết hợp lọc dữ liệu theo các tiêu chí: Kỳ xuất hóa đơn (chọn khoảng thời gian thực tế phát hành), Hình thức hóa đơn, Ký hiệu, Trạng thái gửi CQT, Tính chất hóa đơn, Người lập hóa đơn.

* **Tìm kiếm nhanh**: Nhập số hóa đơn, MST hoặc tên khách hàng vào ô tìm kiếm. Tìm kiếm theo tên khách hàng không phân biệt hoa thường. Tìm kiếm sẽ kết hợp với bộ lọc hiện tại để ra kết quả.

* **Tùy chọn hiển thị:** Nhấn ***Ẩn/Hiện cột***→ tích chọn các cột thông tin cần hiển thị trên màn hình.

* **Xem chi tiết hóa đơn từ báo cáo**:
  + Nhấn vào số hóa đơn trong cột ***Số hóa đơn***.

* Hệ thống chuyển hướng sang màn hình chi tiết hóa đơn trong chức năng Hóa đơn tương ứng.

### **2. Xuất file báo cáo (Tổng quan & Chi tiết)**

**Tình huống:** Bạn cần lưu trữ file dữ liệu hóa đơn bán ra trong quý vừa qua.

**Thao tác thực hiện như sau:** Tại màn hình **Bảng kê tổng hợp** đã lọc đúng kỳ, bạn nhấn ***Xuất file*** → chọn:

* ***Xuất file tổng hợp***để xuất Excel 1 dòng/hóa đơn.

* **Xuất file chi tiết** để xuất Excel 1 dòng/hàng hóa.

* ***Xuất file tổng hợp hàng hóa*** để xuất Excel tổng hợp số liệu theo hàng hóa (mã hàng, ĐVT, thuế suất) thay vì theo hóa đơn.

## **III. Các tính năng nâng cao**

### **Tính tự động của các bộ lọc trạng thái và tính chất**

**Tình huống:** Cửa hàng của bạn đồng thời xuất hóa đơn từ máy tính tiền cho khách lẻ và hóa đơn có mã CQT cho đối tác mua buôn, bạn cần lọc chính xác trạng thái từng loại hóa đơn.

**Thao tác thực hiện như sau:**

* Tại tiêu chí **Tính chất hóa đơn**, khi bạn chọn ***Hóa đơn điều chỉnh*** hoặc ***Hóa đơn thay thế*** → hệ thống tự động hiện thêm bộ lọc con (Chưa lập biên bản/ Người mua chưa ký biên bản/ Người mua đã ký biên bản).

* Bộ lọc **Trạng thái CQT** tự động hiển thị đúng tập trạng thái phù hợp với hình thức hóa đơn đang sử dụng, tránh hiển thị trạng thái không áp dụng, các trạng thái cụ thể như sau:
  + Hóa đơn thường - Có mã: Phát hành lỗi | Đã phát hành | CQT đã cấp mã | Không đủ điều kiện cấp mã.
  + Hóa đơn thường - Không mã: Phát hành lỗi | Đã phát hành | CQT đã tiếp nhận | CQT kiểm tra không hợp lệ.
  + Hóa đơn máy tính tiền: Chưa gửi CQT | Lỗi gửi hóa đơn cho CQT | Đã gửi CQT | CQT đã tiếp nhận | CQT kiểm tra không hợp lệ.
  + Khi đơn vị dùng nhiều hình thức: tổng hợp tất cả trạng thái của các hình thức đang sử dụng.
