...
source_url: https://www.kiotviet.vn/huong-dan-su-dung-kiotviet/retail-giao-dich/hoa-don-dau-vao/
---
...
Tính năng "Hóa đơn đầu vào" cho phép cửa hàng kết nối trực tiếp với hệ thống của Cơ quan Thuế (CQT) để tự động đồng bộ hóa đơn điện tử đầu vào. Thay vì nhập tay phiếu nhập hàng từ hóa đơn giấy hoặc file PDF, giờ đây bạn có thể chuyển đổi trực tiếp hóa đơn đã được xác thực thành phiếu nhập hàng trên KiotViet chỉ với vài thao tác, đảm bảo tính chính xác và hợp lệ của dữ liệu.

**Lợi ích nổi bật của tính năng:**

* **Tiết kiệm thời gian:** Giảm 60-80% thời gian nhập hàng thủ công, loại bỏ sai sót khi nhập liệu số lượng, đơn giá, thuế.
* **Đảm bảo tính pháp lý:** Tự động kiểm tra và chỉ đồng bộ các hóa đơn hợp lệ (có mã CQT, đúng MST) trực tiếp từ Cơ quan Thuế.
* **Giảm rủi ro thuế:** Loại bỏ nguy cơ nhập nhầm hóa đơn giả hoặc không hợp lệ, giúp việc hạch toán và đối soát thuế VAT chính xác.
* **Quản lý tập trung:** Giúp chủ cửa hàng và bộ phận kế toán dễ dàng đối soát công nợ với nhà cung cấp, liên kết chặt chẽ giữa hóa đơn điện tử và phiếu nhập hàng trong KiotViet.

## **II. Các thao tác cơ bản**

Để dễ hình dung, các ví dụ dưới đây sẽ minh họa những tình huống thực tế trong ngành Bán buôn, Bán lẻ.

### **1. Kết nối tài khoản với Cơ quan Thuế (CQT)**

**Tình huống:** Để bắt đầu sử dụng tính năng, bạn cần thiết lập kết nối giữa KiotViet và tài khoản tra cứu hóa đơn của doanh nghiệp bạn trên Cơ quan Thuế.

**Thao tác thực hiện như sau:**

* **Bước 1**: Trên màn hình **Quản lý**, bạn chọn **Mua hàng** → **Hóa đơn đầu vào**.

* **Bước 2**: Lần đầu tiên truy cập tính năng, bạn nhấn ***Kết nối ngay***.

* **Bước 3**: Bạn nhập ***Tên đăng nhập*** và ***Mật khẩu*** (tài khoản Cơ quan Thuế đã cấp cho đơn vị tại https://hoadondientu.gdt.gov.vn/ → tích ***Tôi đồng ý…*** để tự động tải về hoá đơn theo Nghị định 70/2025/NĐ-CP).
* **Bước 4**: Nhấn ***Kết nối ngay*** để hoàn tất.

* **Bước 5**: Tài khoản kết nối thành công hiển thị trong tab **Danh sách kết nối**.  
  **Lưu ý**: Bạn có thể kết nối thêm nhiều tài khoản CQT nếu cần bằng cách nhấn ***Thêm kết nối***.

### **2. Đồng bộ hóa đơn đầu vào**

**Tình huống:** Sau khi kết nối thành công, bạn muốn tải danh sách các hóa đơn đầu vào (hóa đơn mua hàng) từ CQT về KiotViet.

**Thao tác thực hiện như sau:**

* **Bước 1**: Tại màn hình **Hóa đơn đầu vào**, bạn chuyển sang tab **Danh sách hóa đơn**.
* **Bước 2**: Nhấn ***Đồng bộ hóa đơn***.
* **Bước 3**: Chọn tab **Thời gian đồng bộ**.
* **Bước 4**: Chọn ***Mã số thuế*** (với gian hàng có nhiều mã số thuế)
* **Bước 5**: Chọn khoảng thời gian (Từ ngày, Đến ngày) mà bạn muốn đồng bộ hóa đơn (Tối đa 90 ngày/lần chọn).
* **Bước 6**: Nhấn ***Đồng bộ hóa đơn***.

* **Bước 7**: Hệ thống sẽ thực hiện đồng bộ. Sau khi hoàn tất, các hóa đơn mới sẽ xuất hiện với trạng thái ***Mới***.

* **Bước 8**: Bạn có thể xem lại lịch sử các lần đồng bộ trong tab **Lịch sử đồng bộ**.

### **3. Khớp dữ liệu hóa đơn**

**Tình huống:** Khi một hóa đơn mới được đồng bộ về, để tạo phiếu nhập hàng bạn cần kiểm tra và khớp thông tin Nhà cung cấp và Hàng hóa trên hóa đơn với các dữ liệu tương ứng bạn đang quản lý trên KiotViet.

**Thao tác thực hiện như sau:**

* **Bước 1**: Trong **Danh sách hóa đơn**, tìm hóa đơn có trạng thái ***Mới*** →  nhấn ***Lập phiếu*** → chọn ***Phiếu nhập hàng***.

* **Bước 2**: Trên popup **Xác nhận dữ liệu từ hóa đơn điện tử**, bạn thực hiện các thao tác:
  + **Khớp Nhà cung cấp:**
    - Hệ thống sẽ tự động tìm và khớp Nhà cung cấp (NCC) trong KiotViet nếu trùng khớp Mã số thuế (MST) hoặc Tên (trên 80%).
    - Nếu NCC đã khớp, hệ thống hiển thị trạng thái tích v xanh .
    - Nếu chưa khớp, bạn chủ động tìm NCC trong danh sách hoặc nhấn ***+ Thêm mới*** để thêm NCC mới (thông tin Tên và MST sẽ được tự động điền từ hóa đơn).

* Sau khi khớp 1 NCC với 1 NCC trên KiotViet, thì ở các lần map tiếp theo hệ thống sẽ tự động khớp NCC với với NCC trên KiotViet tương tự ở lần liền trước đó.

* **Khớp Hàng hóa:**
  + Tương tự, hệ thống tự động khớp hàng hóa nếu tên hàng khớp trên 80%.
  + Nếu hàng hóa chưa khớp, bạn tìm và chọn hàng hóa tương ứng trong danh mục KiotViet của mình, hoặc nhấn ***+ Thêm mới*** để tạo hàng hóa mới (Tên hàng hóa sẽ được tự động điền từ hóa đơn).

* Sau khi khớp 1 hàng hóa từ NCC với 1 hàng hóa trên KiotViet, thì ở các lần map tiếp theo hệ thống sẽ tự động khớp hàng hóa từ NCC với với hàng hóa trên KiotViet tương tự ở lần liền trước đó.

**Lưu ý**: Bạn phải khớp thành công toàn bộ NCC và tất cả các dòng hàng hóa mới có thể ***Tạo phiếu nhập***.

### **4. Tạo phiếu nhập hàng từ hóa đơn**

**Tình huống:** Sau khi đã khớp toàn bộ dữ liệu ở bước trên, bạn tiến hành tạo phiếu nhập hàng để chính thức ghi nhận hàng hóa vào kho.

**Thao tác thực hiện như sau:**

* **Bước 1**: Tại popup **Xác nhận dữ liệu từ hóa đơn điện tử**, bạn nhấn ***Tạo phiếu nhập***.

* **Bước 2**: Hệ thống chuyển đến màn hình **Nhập hàng** với toàn bộ thông tin (NCC, danh sách hàng hóa, số lượng, đơn giá, thuế) đã được tự động điền từ hóa đơn.
* **Bước 3**: Bạn có thể chỉnh sửa lại Số lượng, Đơn giá, hoặc Giảm giá nếu cần thiết.
* **Bước 4**: **Trường hợp đặc biệt (Hàng Lô/Date, IMEI):** Đối với các hàng hóa quản lý theo Lô hoặc IMEI, Số lượng sẽ hiển thị là "0". Bạn cần nhập thủ công số lượng thực tế và thông tin Lô/IMEI tương ứng.
* **Bước 5**: Bạn nhấn ***Lưu tạm*** (để lưu phiếu tạm) hoặc ***Hoàn thành*** (để hoàn tất phiếu nhập hàng).

* **Bước 6**: Sau khi hoàn thành phiếu nhập hàng → trạng thái Hóa đơn trên **Danh sách hóa đơn** đầu vào sẽ chuyển trạng thái ***Đã lập phiếu***.

## **III. Các tính năng nâng cao**

### **1. Phân quyền tính năng**

**Tình huống:** Trước khi sử dụng tính năng **Hóa đơn đầu vào**, bạn cần được cấp quyền truy cập và các quyền thao tác liên quan (tạo phiếu nhập, tạo hàng hóa,...), được thiết lập tại các mục phân quyền tương ứng.

Tài khoản Admin (chủ gian hàng) mặc định có đầy đủ các quyền và không cần phân quyền thêm.

**Thao tác thực hiện như sau:** Vào **Quản lý** → **Thiết lập** → **Cửa hàng** → **Quản lý người dùng** → khi tạo mới hoặc chỉnh sửa phân quyền:

* **Quyền truy cập tính năng Hóa đơn đầu vào:**
  + Đây là quyền bắt buộc để nhân viên thấy và truy cập tính năng.
  + Trên tab **Phân quyền theo vai trò**, tìm mục ***Hóa đơn đầu vào*** và tích chọn quyền ***Xử lý hóa đơn đầu vào***.

* **Các quyền thao tác liên quan**: Các nút thao tác bên trong tính năng **Hóa đơn đầu vào** sẽ ẩn/hiện tùy thuộc vào các chức năng khác mà nhân viên đó được phân quyền. Cụ thể như sau:
  + Để hiển thị nút ***Tạo phiếu nhập***: Nhân viên cần có quyền ***Nhập hàng*** → ***Tạo phiếu nhập hàng***.

* Để hiển thị nút ***Tạo phiếu chi***: Nhân viên cần có quyền **Sổ quỹ** → ***Tạo phiếu thu chi***.

* Để hiển thị nút ***+ Thêm mới*** nhà cung cấp: Nhân viên cần có quyền **Danh sách nhà cung cấp** → ***Tạo Nhà cung cấp***.

* Để hiển thị nút ***+ Thêm mới*** hàng hóa: Nhân viên cần có quyền **Danh sách hàng hóa** → ***Tạo Hàng hóa***.

### **2. Sửa kết nối**

**Tình huống:** Khi bạn thay đổi mật khẩu tài khoản tra cứu trên trang Cơ quan Thuế, bạn cần cập nhật lại thông tin này trên KiotViet để duy trì kết nối.

**Thao tác thực hiện như sau:**

* **Bước 1**: Trong màn hình **Hóa đơn đầu vào**, bạn chọn tab **Danh sách kết nối**.
* **Bước 2**: Tìm tài khoản muốn sửa, nhấn ***Sửa thiết lập***.
* **Bước 3**: Bạn nhập lại ***Tên đăng nhập*** hoặc ***Mật khẩu*** mới.
* **Bước 4**: Nhấn ***Lưu*** để ghi nhận thay đổi.

### **3. Xóa kết nối**

**Tình huống:** Khi bạn không muốn sử dụng một tài khoản CQT để đồng bộ nữa, bạn có thể xóa kết nối đó khỏi KiotViet.

**Thao tác thực hiện như sau:**

* **Bước 1**: Trong màn hình **Hóa đơn đầu vào**, bạn chọn tab **Danh sách kết nối**.
* **Bước 2**: Tìm tài khoản muốn xóa và nhấn ***Xóa***.
* **Bước 3**: Xác nhận hành động để hoàn tất việc xóa kết nối.

### **4. Lập phiếu chi từ hóa đơn**

**Tình huống:** Nếu hóa đơn này là hóa đơn dịch vụ (không cần nhập kho) hoặc bạn muốn ghi nhận thanh toán ngay, bạn có thể lập phiếu chi trực tiếp thay vì tạo phiếu nhập.

**Thao tác thực hiện như sau:**

* **Bước 1**: Tại tab **Danh sách hóa đơn** trên màn hình **Hóa đơn đầu vào**, bạn nhấn ***Lập phiếu*** → chọn ***Phiếu chi Tiền mặt/chuyển khoản/ví điện tử*** tương ứng với hình thức thanh toán trên hóa đơn.

* **Bước 2**: Hệ thống hiển thị popup **Tạo phiếu chi** với Số tiền được điền tự động theo hóa đơn.
* **Bước 3**: Bạn điền các thông tin còn lại trên phiếu chi và nhấn ***Lưu*** để ghi nhận.
