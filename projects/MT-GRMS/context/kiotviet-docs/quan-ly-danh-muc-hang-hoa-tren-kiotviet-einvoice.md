...
source_url: https://www.kiotviet.vn/huong-dan-su-dung-kiotviet/retail-phat-hanh-hddt/quan-ly-danh-muc-hang-hoa-tren-kiotviet-einvoice/
---
...
Hỗ trợ
Phát hành HĐĐTQuản lý danh mục hàng hóa trên KiotViet eInvoice
...
Module Quản lý danh mục hàng hóa cho phép người dùng lưu trữ, tra cứu và duy trì toàn bộ thông tin sản phẩm/dịch vụ trên phần mềm KiotViet. Dữ liệu hàng hóa này đóng vai trò nền tảng, được dùng để điền tự động khi phát hành hóa đơn điện tử, đảm bảo các thông tin cốt lõi (như tên hàng, đơn giá, VAT, chiết khấu) luôn chính xác và đồng nhất. Module hỗ trợ đầy đủ các thao tác tạo mới, cập nhật, tìm kiếm/lọc, import/export hàng loạt, cũng như quản lý các thuộc tính đặc thù (Số lô/IMEI, dung tích, màu sắc...).

**Lợi ích nổi bật của tính năng:**

* **Điền tự động lên hóa đơn, giảm thiểu sai sót:** Khi lập hóa đơn điện tử, hệ thống sẽ tự động gọi dữ liệu tên hàng, đơn giá, VAT và chiết khấu từ danh mục. Điều này giúp nhân viên tiết kiệm tối đa thời gian nhập liệu và hạn chế rủi ro sai sót trên chứng từ gửi khách hàng.
* **Quản lý tập trung, tra cứu linh hoạt:** Toàn bộ sản phẩm được lưu trữ tập trung với công cụ tìm kiếm real-time và bộ lọc đa dạng (theo nhóm hàng, đơn vị tính, VAT, trạng thái). Các công cụ Import/Export Excel giúp việc khởi tạo và đồng bộ danh mục số lượng lớn trở nên nhanh chóng.
* **Hỗ trợ quản lý chuyên sâu với Số lô/IMEI và Thuộc tính:** Đáp ứng tốt cho các ngành hàng cần truy xuất nguồn gốc (như thực phẩm, dược phẩm, điện tử), hệ thống cho phép gắn theo dõi Số lô/hạn sử dụng hoặc IMEI, đồng thời bổ sung các thuộc tính tùy chỉnh (dung tích, màu sắc...) ngay khi tạo mới sản phẩm.

## **II. Các thao tác cơ bản**

Để dễ hình dung, các ví dụ dưới đây sẽ minh họa những tình huống thực tế trong ngành Bán buôn, Bán lẻ.

### **1. Truy cập và mở form Tạo hàng hóa**

* **Tình huống:** Cửa hàng chuẩn bị kinh doanh một mặt hàng mới, bạn cần thêm sản phẩm này vào hệ thống để nhân viên thu ngân có thể xuất hóa đơn ngay.
* **Thao tác thực hiện như sau:**
  + **Bước 1:** Truy cập menu **Danh mục** → chọn **Hàng hóa**.

* **Bước 2:** Màn hình danh sách **Hàng hóa** sẽ hiện ra. Tại đây, bạn nhấn ***+ Thêm hàng hóa***.

* **Bước 3:** Hệ thống hiển thị popup **Tạo hàng hóa**.

### **2. Nhập thông tin và thiết lập thuộc tính hàng hóa**

* **Tình huống:** Mặt hàng mới này cần được quản lý chặt chẽ về giá bán, thuế VAT và bắt buộc phải theo dõi theo số lô cùng hạn sử dụng.
* **Thao tác thực hiện như sau:**
  + **Bước 1 - Điền thông tin cơ bản bắt buộc**:
    - ***Mã hàng hóa***: tối đa 50 ký tự, không chứa ký tự đặc biệt hoặc khoảng trắng, không được trùng lặp.
    - ***Tên hàng hóa***: tối đa 500 ký tự.
  + **Bước 2 - Thông tin bổ sung**:
    - Chọn ***Nhóm hàng***, ***Đơn vị tính*** và mức ***Tỷ lệ tính thuế (VAT)*** (từ danh sách có sẵn).
    - Nhập ***Đơn giá*** (bằng VND, số dương) và ***Tỷ lệ chiết khấu*** (từ 0-100%) nếu có. Hệ thống sẽ tự động làm tròn phần thập phân theo quy chuẩn hóa đơn.
  + **Bước 3: Thiết lập Số lô/IMEI và Thuộc tính (nếu có)**
    - Nhấn mở rộng mục ***Số lô/IME***I và chọn ***Số lô – hạn sử dụng*** hoặc ***IMEI*** để hệ thống yêu cầu thông tin này khi xuất hóa đơn.
    - Nhấn mở rộng ***Thuộc tính*** và chọn ***+ Thêm thuộc tính*** nếu bạn muốn thêm đặc điểm như dung tích, màu sắc của sản phẩm.

### **3. Lưu dữ liệu và kiểm tra danh sách**

* **Tình huống:** Bạn muốn lưu lại mặt hàng vừa tạo và tiếp tục mở form để nhập nhanh một mặt hàng tiếp theo thuộc cùng lô hàng.
* **Thao tác thực hiện như sau:**
  + **Bước 1 - Lưu dữ liệu**: Trên popup tạo hàng hóa:
    - Nếu bạn chỉ tạo 1 mặt hàng, hãy nhấn ***Lưu***.
    - Nếu muốn tạo liên tục, nhấn nút ***Lưu và tạo thêm hàng*** → hệ thống sẽ lưu sản phẩm hiện tại và mở mới để bạn nhập tiếp mặt hàng khác mà không cần đóng popup.

* **Bước 2 - Kiểm tra kết quả**: Sau khi hoàn tất và nhấn **Lưu** → hàng hóa vừa tạo sẽ xuất hiện ngay ở vị trí đầu tiên của trang danh sách.

## **III. Các tính năng nâng cao**

### **1. Tìm kiếm, lọc và xem chi tiết hàng hóa**

* **Tình huống:** Bạn cần tra cứu lại thông tin đơn giá và VAT của một sản phẩm cũ để báo giá cho khách sỉ, sau đó chỉnh sửa lại mức chiết khấu mới.
* **Thao tác thực hiện như sau:**
  + **Bước 1 - Tìm kiếm nhanh và Lọc**: Sử dụng thanh tìm kiếm trên cùng và nhập mã hàng hoặc tên hàng (hệ thống sẽ hiển thị kết quả ngay khi nhập mà không cần nhấn Enter). Bạn cũng có thể dùng bộ lọc bên trái để thu hẹp kết quả theo Nhóm hàng, Đơn vị tính, VAT hoặc Trạng thái (Hoạt động/Không hoạt động).

* **Bước 2 - Xem và chỉnh sửa chi tiết**: Nhấn vào khách hàng cụ thể trên danh sách để mở ra chi tiết → chọn ***Chỉnh sửa*** → cập nhật thông tin và nhấn ***Lưu***. Mọi thay đổi đều được hệ thống lưu vết (thời gian, người thực hiện, giá trị cũ/mới).

### **2. Quản lý trạng thái và Xóa hàng hóa**

* **Tình huống:** Có một số mặt hàng nhà cung cấp đã ngừng sản xuất, bạn muốn ẩn chúng khỏi danh sách chọn khi xuất hóa đơn hoặc xóa vĩnh viễn các mã hàng tạo nháp.
* **Thao tác thực hiện như sau:**
  + **Bước 1 - Tạm ngừng hoạt động:** 
    - Khi xem chi tiết hàng hóa bạn nhấn ***Ngừng hoạt động*** → xác nhận ***Đồng ý***.

* Mở panel chi tiết của mặt hàng đó và nhấn "Ngừng hoạt động". Sau khi xác nhận, hàng hóa sẽ chuyển sang trạng thái "Không hoạt động" (không hiển thị khi lập hóa đơn nhưng vẫn tìm lại được qua bộ lọc để "Kích hoạt" lại sau này).

* **Bước 2 - Xóa hàng hóa:** Để xóa hàng hóa khỏi danh mục, bạn có thể nhấn ***Xóa*** khi xem chi tiết. Nếu cần xóa hàng loạt, hãy tích chọn các hàng hóa → nhấn ***Xóa hàng hóa*** → nhấn ***Xác nhận***.

### **3. Import / Export hàng hóa hàng loạt bằng Excel**

* **Tình huống:** Cửa hàng mới triển khai phần mềm và bạn đang có một file danh sách hàng trăm sản phẩm cần đẩy lên hệ thống cùng lúc.
* **Thao tác thực hiện như sau:**
  + **Bước 1 - Tải file mẫu và chuẩn bị dữ liệu**: Bạn nhấn ***Import file***, sau đó nhấn vào link Tải về file mẫu: ***Excel file*** trong popup. Điền thông tin hàng hóa của bạn vào file mẫu này theo đúng định dạng cột.

* **Bước 2 - Tải dữ liệu lên hệ thống**: Trên popup, bạn chọn quy tắc xử lý nếu ***Xử lý trùng mã hàng, khác tên hàng hóa*** (Báo lỗi và dừng import hoặc Thay thế tên hàng cũ bằng tên hàng mới). Sau đó, nhấn ***Chọn file dữ liệu*** → kéo thả file hoặc tiếp tục nhấn ***Chọn file*** để đưa thông tin lên hệ thống.

* **Bước 3 -** **Xuất dữ liệu**: Khi cần sao lưu hoặc làm báo cáo, bạn chỉ cần nhấn ***Xuất file*** ở góc trên bên phải màn hình danh sách, hệ thống sẽ tự động kết xuất toàn bộ danh mục hiện tại ra một file Excel chuẩn để bạn tải về.

### **4. Sắp xếp và phân trang danh sách**

* **Tình huống:** Bạn muốn xem lại các mặt hàng có đơn giá từ cao xuống thấp và xem được nhiều mặt hàng hơn trên cùng một màn hình để dễ bao quát.
* **Thao tác thực hiện như sau:**
  + **Bước 1 - Sắp xếp theo cột**: Bạn nhấn vào tiêu đề cột (ví dụ: cột Đơn giá) để sắp xếp tăng dần hoặc giảm dần.

* **Bước 2 - Tùy chỉnh số lượng hiển thị**: Kéo xuống cuối danh sách, tại khu vực phân trang, bạn có thể thay đổi số lượng dòng hiển thị trên một trang từ 10 (mặc định) lên 15, 50 hoặc 100 dòng để thuận tiện hơn cho việc kiểm tra đối chiếu.
